...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  // Check if a message is received on the topic "rpi_1/broadcast"
...
void loop() {
  ...
  std::string temp_str = "Humidity: " + std::to_string(h) + "%  Temperature: " + std::to_string(t) + "°C " + 
  std::to_string(f) + "°F  Heat index: " + std::to_string(hic) + "°C " + std::to_string(hif) + "°F";
  char* temp_buff = new char[temp_str.size() + 1]; // +1 for null terminator
  std::strcpy(temp_buff, temp_str.c_str());
  Serial.println(temp_str);
  client.publish("environment/temperature_1", temp_buff);
  delete[] temp_buff; 

  // Light
  float lux = lightMeter.readLightLevel();
  std::string light_str = "Light: " + std::to_string(lux) + " lx";
  char* light_buff = new char[light_str.size() + 1];
  std::strcpy(light_buff, light_str.c_str());
  Serial.println(light_str);
  client.publish("environment/light_1", light_buff);
  delete[] light_buff; 
